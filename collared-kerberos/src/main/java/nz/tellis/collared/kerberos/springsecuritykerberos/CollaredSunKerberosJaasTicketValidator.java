    ...
    private class KerberosValidateAction implements PrivilegedExceptionAction<KerberosTicketValidation> {
        ...
        public KerberosTicketValidation run() throws Exception {
            ...
            if (context.getCredDelegState()) {
                ...
                LOG.warn("Did not get delegated credential - have you correctly configured the service principal for delegation?");
            }
            if (!holdOnToGSSContext) {
                context.dispose();
            }
            return new CollaredKerberosTicketValidation(new KerberosTicketValidation(gssName.toString(), servicePrincipal,
                    responseToken, context), delegationCredential);
        }
    }
